import { GoogleGenerativeAI } from "@google/generative-ai";
import { db } from "@/lib/prisma";

const API_KEY = process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY;

if (!API_KEY) {
    console.warn("CRITICAL: AI API Key (GOOGLE_API_KEY or GEMINI_API_KEY) is missing from environment variables.");
}

const genAI = new GoogleGenerativeAI(API_KEY || "");
const PRIMARY_MODEL = "gemini-2.5-flash";

export async function generateAIResponse(prompt, context = {}) {
    if (!API_KEY) {
        throw new Error("AI Integration is not configured. Please add GOOGLE_API_KEY to your environment variables.");
    }

    let result;
    let modelUsed = PRIMARY_MODEL;

    try {
        const model = genAI.getGenerativeModel({ model: PRIMARY_MODEL });
        result = await model.generateContent(prompt);
    } catch (error) {
        console.error("Gemini API Error:", error.message);
        
        // Only attempt Ollama fallback in development
        if (process.env.NODE_ENV === "development") {
            console.log("Attempting Ollama fallback...");
            result = await ollamaFallback(prompt);
            modelUsed = "ollama-llama3";
        } else {
            throw new Error("Gemini API failed: " + error.message);
        }
    }

    // Asynchronous logging (fire-and-forget)
    if (context?.userId) {
        let promptTokens = 0;
        let completionTokens = 0;
        let totalTokens = 0;

        if (result.response?.usageMetadata) {
            promptTokens = result.response.usageMetadata.promptTokenCount || 0;
            completionTokens = result.response.usageMetadata.candidatesTokenCount || 0;
            totalTokens = result.response.usageMetadata.totalTokenCount || 0;
        } else {
            // Token estimation for fallbacks
            const responseText = typeof result.response?.text === 'function' ? result.response.text() : "";
            promptTokens = Math.ceil(prompt.length / 4);
            completionTokens = Math.ceil(responseText.length / 4);
            totalTokens = promptTokens + completionTokens;
        }

        db.apiUsage.create({
            data: {
                userId: context.userId,
                feature: context.feature ?? "unknown",
                model: modelUsed,
                promptTokens,
                completionTokens,
                totalTokens,
            }
        }).catch(err => console.error('[ApiUsage] Failed to log:', err));
    }

    return result;
}

async function ollamaFallback(prompt) {
    const ollamaUrl = process.env.OLLAMA_URL || "http://localhost:11434";

    try {
        const response = await fetch(`${ollamaUrl}/api/generate`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({
                model: "llama3", // configurable fallback model
                prompt: prompt,
                stream: false,
            }),
        });

        if (!response.ok) {
            throw new Error(`Ollama request failed with status ${response.status}`);
        }

        const data = await response.json();

        // Format the response to mimic Gemini's structure so upstream code doesn't break
        return {
            response: {
                text: () => data.response,
                candidates: [
                    {
                        content: {
                            parts: [{ text: data.response }],
                        },
                    },
                ],
            },
        };
    } catch (err) {
        console.error("Ollama fallback also failed:", err.message);
        throw new Error("Both primary AI and fallback AI failed. " + err.message);
    }
}

/** Robust JSON Generative Pipeline with Automatic Error Retries */
export async function generateAIJSON(prompt, context = {}, retries = 2) {
    let attempt = 0;
    let lastError = null;
    let currentPrompt = prompt;

    while (attempt <= retries) {
        try {
            const result = await generateAIResponse(currentPrompt, context);
            const text = result.response?.text
                ? result.response.text()
                : (result.response?.candidates?.[0]?.content?.parts?.[0]?.text || "");

            if (!text || text.length < 10) {
                throw new Error("Empty AI response");
            }

            // Clean potential markdown blocks
            const cleanedText = text.replace(/```(?:json)?\n?/g, "").replace(/```\n?/g, "").trim();

            let parsed;
            try {
                parsed = JSON.parse(cleanedText);
            } catch (e) {
                const match = cleanedText.match(/\{[\s\S]*\}/);
                if (match) {
                    parsed = JSON.parse(match[0]);
                } else {
                    throw new Error("Invalid JSON structure from AI");
                }
            }

            return parsed;
        } catch (error) {
            attempt++;
            lastError = error;
            console.warn(`JSON parsing failed on attempt ${attempt}. Retrying...`);
            // Update prompt to heavily mandate strict JSON for the retry
            currentPrompt = `${prompt}\n\nCRITICAL SYSTEM INSTRUCTION: YOUR PREVIOUS RESPONSE WAS INVALID JSON. YOU MUST RETURN **ONLY** STRICTLY VALID, MINIFIED JSON. NO MARKDOWN, NO CONVERSATION, NO OTHER TEXT.`;
        }
    }

    console.error("Failed to generate robust AI JSON after retries:", lastError);
    throw new Error("AI failed to return valid JSON format");
}
